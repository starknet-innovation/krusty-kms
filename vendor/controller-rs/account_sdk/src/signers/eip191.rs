...
/// A signer that implements EIP-191 signing for Ethereum-compatible keys.
...
#[cfg(not(target_arch = "wasm32"))]
fn signing_key_to_eth_address(signing_key: &SigningKey) -> EthAddress {
...
impl Eip191Signer {
    ...
    pub fn random() -> Self {
        ...
        let address = signing_key_to_eth_address(&signing_key);
...
impl HashSigner for Eip191Signer {
    async fn sign(&self, tx_hash: &Felt) -> Result<SignerSignature, SignError> {
        ...
        let message_hash = eth_message_hash(&tx_hash_bytes);
