...
mod tests {
    ...
    #[test]
    fn u256_to_u128_rejects_low_limb_above_u128() {
        let low = Felt::from_hex("0x100000000000000000000000000000000").unwrap();
        let error = u256_to_u128(low, Felt::ZERO).unwrap_err();
        assert!(matches!(error, KmsError::CryptoError(_)));
    }
}
