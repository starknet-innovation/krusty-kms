...
// ============================================================================
// Nostr Messaging (NIP-44 inspired)
// ============================================================================

/// Derive compressed public key (hex) from a secp256k1 secret key (hex).
#[wasm_bindgen(js_name = "nostrPublicKey")]
pub fn nostr_public_key(secret_hex: &str) -> Result<String, JsValue> {
    nostr_messaging::derive_public_key(secret_hex).map_err(|e| JsValue::from_str(&e.to_string()))
}

/// Encrypt a plaintext message using sender secret and recipient public key.
#[wasm_bindgen(js_name = "nostrEncrypt")]
pub fn nostr_encrypt(sender_sk_hex: &str, receiver_pk_hex: &str, message: &str) -> Result<String, JsValue> {
    nostr_messaging::encrypt_message(sender_sk_hex, receiver_pk_hex, message.as_bytes())
        .map_err(|e| JsValue::from_str(&e.to_string()))
}

/// Decrypt a payload produced by `nostrEncrypt`.
#[wasm_bindgen(js_name = "nostrDecrypt")]
pub fn nostr_decrypt(receiver_sk_hex: &str, sender_pk_hex: &str, payload_b64: &str) -> Result<String, JsValue> {
    let bytes = nostr_messaging::decrypt_message(receiver_sk_hex, sender_pk_hex, payload_b64)
        .map_err(|e| JsValue::from_str(&e.to_string()))?;
...
pub fn serialize_public_key(x: &str, y: &str) -> String {
    ...
    ghoul_common::utils::serialize_public_key_hex(&x_felt, &y_felt)
}
