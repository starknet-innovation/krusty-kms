...
fn subtract_points(a: &ProjectivePoint, b: &ProjectivePoint) -> Result<ProjectivePoint> {
    ...
    let b_affine = b.to_affine()
        .map_err(|_| ghoul_common::GhoulError::CryptoError("Invalid point (identity)".to_string()))?;

    let neg_y = Felt::ZERO - b_affine.y();
    let neg_b = ProjectivePoint::from_affine(b_affine.x(), neg_y)
        .map_err(|_| ghoul_common::GhoulError::CryptoError("Invalid negated point".to_string()))?;
...
fn discrete_log_brute_force(g_m: &ProjectivePoint) -> Result<u128> {
    // Use the standard Stark curve generator from she-core
    ...
    Err(ghoul_common::GhoulError::CryptoError(
