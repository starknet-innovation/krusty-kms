...
impl TongoContract {
    ...
    pub async fn get_state(&self, public_key: &ProjectivePoint) -> Result<AccountState> {
        let affine = public_key
            .to_affine()
            .map_err(|_| ghoul_common::GhoulError::CryptoError("Invalid public key".to_string()))?;
            ...
            .call(
                FunctionCall {
                    ...
                    entry_point_selector: get_selector_from_name("get_state")
                        .map_err(|e| ghoul_common::GhoulError::CryptoError(e.to_string()))?,
                    calldata,
                },
                BlockId::Tag(BlockTag::Latest),
            )
            .await
            .map_err(|e| ghoul_common::GhoulError::RpcError(e.to_string()))?;
        ...
        if result.len() < 9 {
            return Err(ghoul_common::GhoulError::DeserializationError(
        ...
        let balance_l = ProjectivePoint::from_affine(rs_felt_to_core(result[0]), rs_felt_to_core(result[1]))
            .map_err(|_| ghoul_common::GhoulError::DeserializationError("Invalid balance.L point".to_string()))?;
        let balance_r = ProjectivePoint::from_affine(rs_felt_to_core(result[2]), rs_felt_to_core(result[3]))
            .map_err(|_| ghoul_common::GhoulError::DeserializationError("Invalid balance.R point".to_string()))?;

        // Deserialize pending
        let pending_l = ProjectivePoint::from_affine(rs_felt_to_core(result[4]), rs_felt_to_core(result[5]))
            .map_err(|_| ghoul_common::GhoulError::DeserializationError("Invalid pending.L point".to_string()))?;
        let pending_r = ProjectivePoint::from_affine(rs_felt_to_core(result[6]), rs_felt_to_core(result[7]))
            .map_err(|_| ghoul_common::GhoulError::DeserializationError("Invalid pending.R point".to_string()))?;
    ...
    pub async fn auditor_key(&self) -> Result<Option<ProjectivePoint>> {
        ...
        if result[0] == StarknetRsFelt::ZERO {
            ...
            let point = ProjectivePoint::from_affine(rs_felt_to_core(result[1]), rs_felt_to_core(result[2]))
                .map_err(|_| ghoul_common::GhoulError::DeserializationError("Invalid auditor key point".to_string()))?;

            return Ok(Some(point));
        }

        Err(ghoul_common::GhoulError::DeserializationError(
