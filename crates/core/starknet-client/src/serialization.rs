...
use ghoul_common::{AuditProof, ElGamalCiphertext, ElGamalProof, Poe2Proof, PoeProof, ProofOfBit, ProofOfTransfer, Range, Result};
...
pub fn serialize_projective_point(point: &ProjectivePoint) -> Result<(Felt, Felt)> {
    let affine = point
        .to_affine()
        .map_err(|_| ghoul_common::GhoulError::CryptoError("Invalid point".to_string()))?;
...
pub fn deserialize_projective_point(x: Felt, y: Felt) -> Result<ProjectivePoint> {
    ProjectivePoint::from_affine(x, y)
        .map_err(|_| ghoul_common::GhoulError::CryptoError("Invalid point coordinates".to_string()))
}
...
pub fn serialize_poe_proof(proof: &PoeProof) -> Result<Vec<Felt>> {
    ...
    let a_x = Felt::from_hex(&proof.a.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid point x: {}", e)))?;
    let a_y = Felt::from_hex(&proof.a.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid point y: {}", e)))?;

    // Convert s string to Felt (s is hex format like "0x123abc")
    let s_felt = Felt::from_hex(&proof.s)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid s scalar: {}", e)))?;
...
pub fn serialize_poe2_proof(proof: &Poe2Proof) -> Result<Vec<Felt>> {
    ...
    let s1_felt = Felt::from_hex(&proof.s1)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid s1 scalar: {}", e)))?;
    let s2_felt = Felt::from_hex(&proof.s2)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid s2 scalar: {}", e)))?;
...
pub fn serialize_elgamal_proof(proof: &ElGamalProof) -> Result<Vec<Felt>> {
    ...
    let al_x = Felt::from_hex(&proof.al.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AL.x: {}", e)))?;
    let al_y = Felt::from_hex(&proof.al.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AL.y: {}", e)))?;

    // Convert AR point
    let ar_x = Felt::from_hex(&proof.ar.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AR.x: {}", e)))?;
    let ar_y = Felt::from_hex(&proof.ar.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AR.y: {}", e)))?;

    // Convert sb and sr strings to Felts (hex format)
    let sb_felt = Felt::from_hex(&proof.sb)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid sb scalar: {}", e)))?;
    let sr_felt = Felt::from_hex(&proof.sr)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid sr scalar: {}", e)))?;
...
pub fn serialize_audit_proof(proof: &AuditProof) -> Result<Vec<Felt>> {
    ...
    let ax_x = Felt::from_hex(&proof.ax.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid Ax.x: {}", e)))?;
    let ax_y = Felt::from_hex(&proof.ax.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid Ax.y: {}", e)))?;

    // Convert AL0 point
    let al0_x = Felt::from_hex(&proof.al0.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AL0.x: {}", e)))?;
    let al0_y = Felt::from_hex(&proof.al0.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AL0.y: {}", e)))?;

    // Convert AL1 point
    let al1_x = Felt::from_hex(&proof.al1.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AL1.x: {}", e)))?;
    let al1_y = Felt::from_hex(&proof.al1.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AL1.y: {}", e)))?;

    // Convert AR1 point
    let ar1_x = Felt::from_hex(&proof.ar1.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AR1.x: {}", e)))?;
    let ar1_y = Felt::from_hex(&proof.ar1.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid AR1.y: {}", e)))?;

    // Convert scalars sx, sb, sr to Felts (hex format)
    let sx_felt = Felt::from_hex(&proof.sx)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid sx scalar: {}", e)))?;
    let sb_felt = Felt::from_hex(&proof.sb)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid sb scalar: {}", e)))?;
    let sr_felt = Felt::from_hex(&proof.sr)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid sr scalar: {}", e)))?;
...
pub fn serialize_bit_proof(proof: &ProofOfBit) -> Result<Vec<Felt>> {
    ...
    let a0_x = Felt::from_hex(&proof.a0.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid A0.x: {}", e)))?;
    let a0_y = Felt::from_hex(&proof.a0.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid A0.y: {}", e)))?;

    // Convert A1 point
    let a1_x = Felt::from_hex(&proof.a1.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid A1.x: {}", e)))?;
    let a1_y = Felt::from_hex(&proof.a1.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid A1.y: {}", e)))?;

    // Convert c0, s0, and s1 strings to Felts (hex format)
    let c0_felt = Felt::from_hex(&proof.c0)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid c0 scalar: {}", e)))?;
    let s0_felt = Felt::from_hex(&proof.s0)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid s0 scalar: {}", e)))?;
    let s1_felt = Felt::from_hex(&proof.s1)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid s1 scalar: {}", e)))?;
...
pub fn serialize_range(range: &Range) -> Result<Vec<Felt>> {
    ...
    for commitment in &range.commitments {
        let x = Felt::from_hex(&commitment.x)
            .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid commitment.x: {}", e)))?;
        let y = Felt::from_hex(&commitment.y)
            .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid commitment.y: {}", e)))?;
        felts.push(x);
        felts.push(y);
    }
...
pub fn serialize_proof_of_transfer(proof: &ProofOfTransfer) -> Result<Vec<Felt>> {
    ...
    for scalar_str in [&proof.s_x, &proof.s_r, &proof.s_b, &proof.s_b2, &proof.s_r2] {
        let scalar_felt = Felt::from_hex(scalar_str)
            .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid scalar: {}", e)))?;
        felts.push(scalar_felt);
    }

    // Serialize R_aux (2 felts)
    let r_aux_x = Felt::from_hex(&proof.r_aux.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid R_aux.x: {}", e)))?;
    let r_aux_y = Felt::from_hex(&proof.r_aux.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid R_aux.y: {}", e)))?;
    felts.push(r_aux_x);
    felts.push(r_aux_y);
    ...
    let r_aux2_x = Felt::from_hex(&proof.r_aux2.x)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid R_aux2.x: {}", e)))?;
    let r_aux2_y = Felt::from_hex(&proof.r_aux2.y)
        .map_err(|e| ghoul_common::GhoulError::CryptoError(format!("Invalid R_aux2.y: {}", e)))?;
    felts.push(r_aux2_x);
    felts.push(r_aux2_y);
...
mod tests {
    ...
    fn test_serialize_poe_proof() {
        use ghoul_common::SerializablePoint;
