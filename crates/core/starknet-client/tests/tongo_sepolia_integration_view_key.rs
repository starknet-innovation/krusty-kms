...
use tongo_sdk::operations::{fund, rollover, transfer, withdraw, ragequit, FundParams, RolloverParams, TransferParams, WithdrawParams, RagequitParams};
use tongo_sdk::{AccountState, TongoAccount};
...
async fn test_full_tongo_sepolia_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // -------------------------------------------------------------------------
    // Step 3: Derive TONGO keys (coin type 5454) and VIEW keys (coin type 5353)
    // -------------------------------------------------------------------------
    ...
    let initial_balance = decrypt_cipher_balance(&tongo_keypair_0.private_key, &initial_state.balance)?;
    let initial_pending = decrypt_cipher_balance(&tongo_keypair_0.private_key, &initial_state.pending)?;
    ...
    let (hint_ciphertext, hint_nonce) = build_view_hint(new_balance_plain, &view_keypair_0.private_key);
    ...
    if should_execute {
        ...
        let private_key_bytes = account_keypair.private_key.to_bytes_be();
        ...
        let new_balance = decrypt_cipher_balance(&tongo_keypair_0.private_key, &new_state.balance)?;
        // View-only: decrypt the hint using ONLY the viewing key (no spending key)
        let viewed_new_balance = decrypt_view_hint(&hint_ciphertext, &hint_nonce, &view_keypair_0.private_key)?;
        ...
        let owner_attempt = decrypt_view_hint(&hint_ciphertext, &hint_nonce, &tongo_keypair_0.private_key)?;
    ...
    let updated_balance = decrypt_cipher_balance(&tongo_keypair_0.private_key, &updated_state.balance)?;
    ...
    let (hint_transfer_ct, hint_transfer_nonce) = build_view_hint(transfer_amount, &view_keypair_1.private_key);
    let leftover_after_transfer = updated_balance.saturating_sub(transfer_amount);
    let (hint_leftover_ct, hint_leftover_nonce) = build_view_hint(leftover_after_transfer, &view_keypair_0.private_key);
    ...
    if should_execute {
        ...
        let final_balance = decrypt_cipher_balance(&tongo_keypair_0.private_key, &final_state.balance)?;
        // View-only: sender can view leftover via view-key hint
        let viewed_leftover = decrypt_view_hint(&hint_leftover_ct, &hint_leftover_nonce, &view_keypair_0.private_key)?;
        ...
        let owner_attempt = decrypt_view_hint(&hint_leftover_ct, &hint_leftover_nonce, &tongo_keypair_0.private_key)?;
    ...
    let account1_balance = decrypt_cipher_balance(&tongo_keypair_1.private_key, &account1_state.balance)?;
    let account1_pending = decrypt_cipher_balance(&tongo_keypair_1.private_key, &account1_state.pending)?;
    // View-only: recipient can view transfer amount via view-key hint
    let viewed_transfer_amount = decrypt_view_hint(&hint_transfer_ct, &hint_transfer_nonce, &view_keypair_1.private_key)?;
    ...
    let (hint_rollover_ct, hint_rollover_nonce) = build_view_hint(post_rollover_plain, &view_keypair_1.private_key);
    ...
    if should_execute {
        ...
        let post_rollover_balance = decrypt_cipher_balance(&tongo_keypair_1.private_key, &post_rollover_state.balance)?;
        let post_rollover_pending = decrypt_cipher_balance(&tongo_keypair_1.private_key, &post_rollover_state.pending)?;
    ...
    let withdraw_balance = decrypt_cipher_balance(&tongo_keypair_1.private_key, &withdraw_state.balance)?;
    ...
    let (hint_withdraw_ct, hint_withdraw_nonce) = build_view_hint(leftover_after_withdraw, &view_keypair_1.private_key);
    ...
    if should_execute {
        ...
        let pre_withdraw_balance = decrypt_cipher_balance(&tongo_keypair_1.private_key, &pre_withdraw_state.balance)?;
        ...
        let post_withdraw_balance = decrypt_cipher_balance(&tongo_keypair_1.private_key, &post_withdraw_state.balance)?;
        // View-only: check leftover using view key hint
        let viewed_after_withdraw = decrypt_view_hint(&hint_withdraw_ct, &hint_withdraw_nonce, &view_keypair_1.private_key)?;
        ...
        let owner_attempt = decrypt_view_hint(&hint_withdraw_ct, &hint_withdraw_nonce, &tongo_keypair_1.private_key)?;
    ...
    let current_balance = decrypt_cipher_balance(&tongo_keypair_1.private_key, &ragequit_state.balance)?;
    ...
    let (hint_ct, hint_nonce) = build_view_hint(0, &view_keypair_1.private_key);
    ...
    if should_execute {
        ...
        let post_ragequit_balance = decrypt_cipher_balance(&tongo_keypair_1.private_key, &post_ragequit_state.balance)?;
        // View-only: post ragequit balance via view-key hint is 0
        let viewed_after_ragequit = decrypt_view_hint(&hint_ct, &hint_nonce, &view_keypair_1.private_key)?;
        ...
        let owner_attempt = decrypt_view_hint(&hint_ct, &hint_nonce, &tongo_keypair_1.private_key)?;
...
// -----------------------------------------------------------------------------
// View-Key Hint Helpers
// -----------------------------------------------------------------------------

/// Build a deterministic view-only hint (ciphertext, nonce) for a given u128
/// plaintext using a keystream derived from the viewing private key.
///
...
fn build_view_hint(amount: u128, view_private_key: &starknet_types_core::felt::Felt) -> ([u8; 64], [u8; 24]) {
    ...
    nonce[8..16].copy_from_slice(&amount_bytes[0..8]);
    // Derive keystream and encrypt 16-byte amount
    let mut ct = [0u8; 64];
    let ks = derive_keystream(view_private_key, &nonce, 16);
    for i in 0..16 {
        ct[i] = amount_bytes[i] ^ ks[i];
    }
    (ct, nonce)
}

fn decrypt_view_hint(
    ciphertext: &[u8; 64],
    nonce: &[u8; 24],
...
fn derive_keystream(
    ...
    use she_core::poseidon_hash_many;
    use starknet_types_core::felt::Felt;

    let mut out = Vec::with_capacity(len);
    let mut counter: u64 = 0;
    while out.len() < len {
        // Pack 24-byte nonce into a Felt (big-endian)
