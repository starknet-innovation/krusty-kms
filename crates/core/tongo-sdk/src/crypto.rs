...
pub fn derive_shared_secret(
    ...
    let affine = shared_point
        .to_affine()
        .map_err(|_| GhoulError::PointAtInfinity)?;
...
pub fn encrypt_audit_hint(
    ...
    she_core::fill_random_bytes(&mut nonce_bytes);

    // Create the cipher
    let cipher = XChaCha20Poly1305::new_from_slice(shared_secret)
        .map_err(|e| GhoulError::CryptoError(format!("Invalid key: {}", e)))?;
    ...
    let ciphertext = cipher
        ...
        .map_err(|e| GhoulError::CryptoError(format!("Encryption failed: {}", e)))?;
...
pub fn decrypt_audit_hint(
    ...
    let plaintext = cipher
        ...
        .map_err(|e| GhoulError::CryptoError(format!("Decryption failed: {}", e)))?;

    // Convert back to u128
    if plaintext.len() != 16 {
        return Err(GhoulError::CryptoError(format!(
