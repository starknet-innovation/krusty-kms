...
//!
//! ```ignore
//! use tongo_sdk::{TongoAccount, operations::{fund, FundParams}};
...
//! let current_balance = ghoul_common::ElGamalCiphertext { l: g.clone(), r: g };
//!
//! // Fund account
...
use ghoul_common::{AuditProof, ElGamalCiphertext, GhoulError, ProofOfTransfer, Result};
use she_core::{
...
pub fn fund(account: &TongoAccount, params: FundParams) -> Result<FundProof> {
    ...
    let y_affine = y.to_affine().map_err(|_| GhoulError::PointAtInfinity)?;
...
pub fn transfer(account: &TongoAccount, params: TransferParams) -> Result<TransferProof> {
    ...
    if !account.has_sufficient_balance(params.amount) {
        return Err(GhoulError::InsufficientBalance {
    ...
    // Get affine coordinates for prefix computation
    let y_affine = y.to_affine().map_err(|_| GhoulError::PointAtInfinity)?;
    let to_affine = to.to_affine().map_err(|_| GhoulError::PointAtInfinity)?;
    ...
    let kx = she_core::scalar::random_felt();
    let kb = she_core::scalar::random_felt();
    let kr = she_core::scalar::random_felt();
    let kb2 = she_core::scalar::random_felt();
    let kr2 = she_core::scalar::random_felt();
    ...
    let challenge = she_core::hash::compute_poseidon_challenge(
        &prefix,
        &[&a_x, &a_r, &a_r2, &a_b, &a_b2, &a_v, &a_v2, &a_bar],
    )?;

    // Compute 5 scalar responses s = k + value * c (transfer.ts:158-162)
    let s_x = she_core::scalar::scalar_add(&kx, &she_core::scalar::scalar_mul(&challenge, x)?)?;
    let s_b = she_core::scalar::scalar_add(
        &kb,
        &she_core::scalar::scalar_mul(&challenge, &Felt::from(b))?,
    )?;
    let s_r = she_core::scalar::scalar_add(&kr, &she_core::scalar::scalar_mul(&challenge, &r)?)?;
    let s_b2 = she_core::scalar::scalar_add(
        &kb2,
        &she_core::scalar::scalar_mul(&challenge, &Felt::from(b_left))?,
    )?;
    let s_r2 = she_core::scalar::scalar_add(&kr2, &she_core::scalar::scalar_mul(&challenge, &r2)?)?;

    // Assemble ProofOfTransfer (transfer.ts:164-182)
    let proof = ProofOfTransfer {
        a_x: ghoul_common::SerializablePoint::try_from_projective(&a_x)?,
        a_r: ghoul_common::SerializablePoint::try_from_projective(&a_r)?,
        a_r2: ghoul_common::SerializablePoint::try_from_projective(&a_r2)?,
        a_b: ghoul_common::SerializablePoint::try_from_projective(&a_b)?,
        a_b2: ghoul_common::SerializablePoint::try_from_projective(&a_b2)?,
        a_v: ghoul_common::SerializablePoint::try_from_projective(&a_v)?,
        a_v2: ghoul_common::SerializablePoint::try_from_projective(&a_v2)?,
        a_bar: ghoul_common::SerializablePoint::try_from_projective(&a_bar)?,
        s_x: format!("{s_x:#x}"),
        s_r: format!("{s_r:#x}"),
        s_b: format!("{s_b:#x}"),
        s_b2: format!("{s_b2:#x}"),
        s_r2: format!("{s_r2:#x}"),
        r_aux: ghoul_common::SerializablePoint::try_from_projective(&r_aux)?,
        range,
        r_aux2: ghoul_common::SerializablePoint::try_from_projective(&r_aux2)?,
        range2,
    };
...
pub fn withdraw(account: &TongoAccount, params: WithdrawParams) -> Result<WithdrawProof> {
    ...
    if g_b_affine != expected_g_b_affine {
        return Err(GhoulError::CryptoError(
    ...
    let (r_aux_result, random_and_commitments) = rayon::join(
        ...
        || {
            ...
            use she_core::random::random_felts;
...
pub struct FundProof {
    ...
    pub proof: ghoul_common::PoeProof,
    pub amount: u128,
...
pub struct WithdrawProof {
    ...
    pub range: ghoul_common::Range, // Range proof for leftover balance
    pub amount: u128,
    pub recipient: Felt,
