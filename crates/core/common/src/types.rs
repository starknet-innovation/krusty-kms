...
impl SerializablePoint {
    ...
    ///
    /// # Errors
    /// Returns `GhoulError::PointAtInfinity` if the point is at infinity.
    pub fn try_from_projective(point: &ProjectivePoint) -> crate::Result<Self> {
        let affine = point
            .to_affine()
            .map_err(|_| crate::GhoulError::PointAtInfinity)?;
    ...
    pub fn to_affine(&self) -> crate::Result<AffinePoint> {
        let x = Felt::from_hex(&self.x)
            .map_err(|e| crate::GhoulError::DeserializationError(e.to_string()))?;
        let y = Felt::from_hex(&self.y)
            .map_err(|e| crate::GhoulError::DeserializationError(e.to_string()))?;

        AffinePoint::new(x, y)
            .map_err(|e| crate::GhoulError::InvalidPublicKey(format!("Invalid point: {:?}", e)))
    }
}
