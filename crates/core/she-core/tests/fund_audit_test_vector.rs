...
fn test_fund_with_audit_typescript_vector() {
    ...
    let ax_rec = ghoul_common::SerializablePoint {
        x: audit_proof.ax.x.clone(),
        y: audit_proof.ax.y.clone(),
    }.to_affine().unwrap().try_into().unwrap();
    let al0_rec: starknet_types_core::curve::ProjectivePoint = ghoul_common::SerializablePoint {
        x: audit_proof.al0.x.clone(),
        y: audit_proof.al0.y.clone(),
    }.to_affine().unwrap().try_into().unwrap();
    let al1_rec: starknet_types_core::curve::ProjectivePoint = ghoul_common::SerializablePoint {
        x: audit_proof.al1.x.clone(),
        y: audit_proof.al1.y.clone(),
    }.to_affine().unwrap().try_into().unwrap();
    let ar1_rec: starknet_types_core::curve::ProjectivePoint = ghoul_common::SerializablePoint {
    ...
    let fund_a_affine = ghoul_common::SerializablePoint {
