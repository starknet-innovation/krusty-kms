...
| Function-size ratchet | `.github/scripts/check-code-complexity.py` | Existing functions over 80 lines cannot grow; new functions must remain at or below 80 lines |
| Cognitive complexity | Clippy `cognitive_complexity` | New control-flow complexity over Clippy's threshold fails the build |
| Scoped coverage floors | `.github/scripts/check-coverage-floors.sh` | Client crate and RPC-adapter directory coverage cannot fall below committed line floors |
