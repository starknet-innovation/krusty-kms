...
- Pure local operations complete with `state = "Completed"`
- Submitted deploys use `state = { "Submitted": { "tx_hash": "0x..." } }`
- Accepted deploys use `state = { "Accepted": { "tx_hash": "0x..." } }`
- Expired tracked operations remain queryable with `state = "Expired"`
- `get_operation_status` returns an explicit tagged lookup result:
  - `{ "status": "found", "operation": ... }`
  - `{ "status": "not_found", "operation_id": "..." }`
